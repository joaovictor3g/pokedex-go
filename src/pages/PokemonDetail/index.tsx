...
export function PokemonDetail() {
  ...
  function adjustStatsParameters(stat: string) {
    ...
    else if (stat === "attack") return "Attack";
    else if (stat === "defense") return "Defense";
    else if (stat === "special-attack") return "Sp. Atk";
    else if (stat === "special-defense") return "Sp. Def";
    else if (stat === "speed") return "Speed";
  }
      ...
      <Container>
        <PokemonView color={colors ? colors[0] : "#666"}>
          <PokemonProfile>
            ...
            <PokemonNameAndId>
              <NormalText color="#fff" fontSize={26} fontWeight>
          ...
          <PokemonInfoBox>
            {id < 10 && (
              <PokeImage
                source={{
                  uri: `https://raw.githubusercontent.com/HybridShivam/Pokemon/master/assets/images/00${id}.png`,
                }}
              />
            )}
            {id >= 10 && id < 100 && (
              <PokeImage
                source={{
                  uri: `https://raw.githubusercontent.com/HybridShivam/Pokemon/master/assets/images/0${id}.png`,
                }}
              />
            )}
            {id >= 100 && (
              <PokeImage
                source={{
                  uri: `https://raw.githubusercontent.com/HybridShivam/Pokemon/master/assets/images/${id}.png`,
                }}
              />
            )}
            <TypeView>
              {serializedTypes.map((serializedType, idx: number) => (
                ...
                >
                  ...
                  />
                  <NormalText
                    color={colors ? colors[idx] : undefined}
                    fontSize={20}
                  >
                    {capitalizeFirstLetter(serializedType)}
                  </NormalText>
                </Type>
              ))}
            </TypeView>
            ...
            <ActionButtons>
              ...
              >
                <NormalText color="#000" fontSize={18}>
                  Sobre
                </NormalText>
              </Button>
              <Button
                ...
                {stats.map((stat, idx: number) => (
                  <Stats key={stat.stat.name}>
                    <NormalText
                      ...
                      style={{ width: "20%" }}
                    >
                      {adjustStatsParameters(stat.stat.name)}
                    </NormalText>
                    <NormalText color="#6666" fontSize={18}>
                      {stat.base_stat}
                    </NormalText>
                    ...
                    <ProgressStatsBar>
                      <View
                        style={{
                          ...
                          backgroundColor: nameTypesAndColors[idx].color,
                          height: 15,
                        }}
                      />
                    </ProgressStatsBar>
